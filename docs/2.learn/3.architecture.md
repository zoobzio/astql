...
|----------|--------|-------|
| PostgreSQL | `github.com/zoobzio/astql/pkg/postgres` | Full feature support, default |
| SQLite | `github.com/zoobzio/astql/pkg/sqlite` | Rejects unsupported features |
| MySQL | `github.com/zoobzio/astql/pkg/mysql` | ON DUPLICATE KEY UPDATE, no RETURNING |
...
result, err := query.RenderWith(sqlite.New())
```
...
| Feature | PostgreSQL | SQLite | MySQL | SQL Server |
|---------|------------|--------|-------|------------|
...
| RETURNING | `RETURNING` | `RETURNING` | Unsupported | `OUTPUT` |

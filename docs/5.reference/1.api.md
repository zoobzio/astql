...
Builds and renders the query to SQL using the default PostgreSQL provider.
...
Builds and renders the query using a specific provider (e.g., `sqlite.New()`).
...
Builds and renders with a specific provider or panics on error.
...
result, err := query.RenderWith(renderer)
```

Full feature support. Used by default when calling `query.Render()`.
...
- `ILIKE` maps to `LIKE` (MySQL LIKE is case-insensitive by default)
...
```go
import "github.com/zoobzio/astql/pkg/mssql"

...
Creates a JSONB text extraction field (`field->>'key'`). PostgreSQL only. Panics if key contains invalid characters (only alphanumeric, underscore, and hyphen allowed).
...
Creates a JSONB path access field (`field->'key'`). PostgreSQL only. Panics if key contains invalid characters. Use with `ArrayContains` for JSONB array queries.
